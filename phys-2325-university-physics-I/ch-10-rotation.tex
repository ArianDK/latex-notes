...
\begin{itemize}
    \item The kinetic energy $K$ of a rigid body rotating about a fixed axis is given by
    \[
        K = \tfrac{1}{2} I \omega^2 \quad \text{(radian measure)},
    \]
    where $I$ is the rotational inertia (moment of inertia) of the body.

    \item For a system of discrete particles, the rotational inertia is defined as
    \[
        I = \sum m_i r_i^2,
    \]
...
\begin{itemize}
    \item The rotational inertia $I$ of a body is defined as
    \[
        I = \sum m_i r_i^2
    \]
    ...
    In these expressions, $r$ (or $r_i$) is the perpendicular distance from the axis of rotation to each mass element.  
    The integration extends over the entire body to include all mass elements.

    \item The parallel-axis theorem relates the rotational inertia $I$ of a body about any axis to that of the same body about a parallel axis through the center of mass:
    \[
        I = I_{\text{com}} + Mh^2,
    \]
    where $h$ is the perpendicular distance between the two axes, $I_{\text{com}}$ is the rotational inertia about the axis through the center of mass, and $M$ is the total mass of the body.  
    The quantity $h$ represents how far the actual rotation axis is shifted from the center-of-mass axis.
\end{itemize}
\subsection{Torque}
\begin{itemize}
    \item Torque is a turning or twisting action on a body about a rotation axis due to a force $\vec{F}$.  
    If a force $\vec{F}$ is applied at a point with position vector $\vec{r}$ relative to the axis, the magnitude of the torque $\tau$ is
    \[
        \tau = r F_t = r_\perp F = rF \sin\phi,
    \]
    where $F_t$ is the component of $\vec{F}$ perpendicular to $\vec{r}$, and $\phi$ is the angle between $\vec{r}$ and $\vec{F}$.

    \item The quantity $r_\perp$ is the perpendicular distance between the rotation axis and the line of action of $\vec{F}$.  
    This distance is called the \textit{moment arm} of $\vec{F}$.  
    Similarly, $r$ is the moment arm of $F_t$.

    \item The SI unit of torque is the newton-meter (N·m).  
    A torque $\tau$ is positive if it tends to rotate a body counterclockwise, and negative if it tends to rotate the body clockwise.
\end{itemize}
\subsection{Newton's second law for rotation}
\begin{itemize}
    \item The rotational analog of Newton’s second law is
    \[
        \tau_{\text{net}} = I \alpha,
    \]
    where $\tau_{\text{net}}$ is the net torque acting on a particle or rigid body,  
    $I$ is the rotational inertia (moment of inertia) of the particle or body about the rotation axis,  
    and $\alpha$ is the resulting angular acceleration about that axis.
\end{itemize}
